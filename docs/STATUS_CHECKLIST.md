...
29. Chart Settings window (new, per PS's spec docs/CHART_SETTINGS_FULL_SPEC.txt)
    ...
    UPGRADE (PS requested, 2026-09-13): replaced the hex-only color picker with a full HSV picker (commit a169fec) - a draggable saturation/value square, a draggable hue spectrum bar, and a row of 10 muted professional preset swatches (click to apply instantly), alongside the existing hex textbox and shade% slider. All controls stay in sync with each other. PLEASE VERIFY: open any color picker, drag on the square and the hue bar, click a few presets, and confirm the swatch/hex/live chart preview all update correctly and stay in sync.
------------------------------------------------------------
